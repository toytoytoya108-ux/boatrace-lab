...
- ヘッダに買い方トグル（絞り込み／15点）。ホーム・レース・成績・実戦投入判定がすべてトグルに従う（localStorage に記憶）。
- レース一覧：場チップ、推奨/見送り/全部、結果（的中/外れ/未確定）、並べ替え（締切/信頼度/期待値/損益）、
  信頼度・期待値の下限スライダー。一覧の確定分の投資/払戻/回収率を表示。
- レース詳細：買い目を「1着 → 2着 → 3着の並び」で表示（フォーメーション・流しの入力順）。1着/2着/3着候補の
  目安と総当たり点数との差を明示。一覧（確率・オッズ・期待値）は折りたたみ。もう一方の買い方の結果も1行表示。
- 成績：場・期間（全期間/今月/直近7日/今日）の絞り込み。
- 設定：絞り込み型のパラメータと実戦投入条件（絞り込み型用）を追加。

## 追補（2026-09-07）レース一覧の場優先ナビゲーションと買い方表示
- レースタブは「場一覧」（場ごとのカード：グレード、進行、予想済み数、推奨数★、確定分の的中/損益）から入り、
  タップでその場の全レース一覧（締切順）へ。先頭の「全場の推奨レース」カードで横断表示。戻る＝場一覧。
- 一覧の並べ替え・信頼度/期待値スライダーは「並べ替え・詳細条件」に畳み、適用中だけ自動で開く。
- 買い方（絞り込み型＝推奨用／15点固定＝比較用）はヘッダーの2段ラベル切替に加え、各画面上部の帯で明示。
  買い方ごとにアクセント色を変える（絞り込み型＝青緑、15点固定＝茶）。
- 設定タブは節ごとに折りたたみ（絞り込み型とデータ書き出しのみ既定で展開）。
